...
pub trait PodProxy: Send + Sync {
    /// Stream a pod's logs to `w` (plain bytes; follow keeps streaming until
    /// the container exits). Err with NotFound if the pod isn't running here.
    fn logs(&self, ns: &str, pod: &str, opts: &LogOpts, w: &mut dyn Write) -> std::io::Result<()>;

    /// Start `cmd` in the pod. The apiserver wires WebSocket stdin/stdout/
    /// stderr/resize channels to the returned handle.
    fn exec_start(
        &self,
        ns: &str,
        pod: &str,
