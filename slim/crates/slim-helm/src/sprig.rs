...
pub fn register(t: &mut Template) {
    ...
    t.add_func("quote", |a| {
        Ok(Value::String(format!(
            "\"{}\"",
            s(a.first().unwrap_or(&Value::Null))
        )))
    });
    t.add_func("squote", |a| {
        Ok(Value::String(format!(
    ...
    t.add_func("upper", |a| {
        Ok(Value::String(
            s(a.first().unwrap_or(&Value::Null)).to_uppercase(),
        ))
    });
    t.add_func("lower", |a| {
        Ok(Value::String(
            s(a.first().unwrap_or(&Value::Null)).to_lowercase(),
        ))
    });
    t.add_func("title", |a| {
        Ok(Value::String(title_case(&s(a
            .first()
            .unwrap_or(&Value::Null)))))
    });
    t.add_func("trim", |a| {
        Ok(Value::String(
            s(a.first().unwrap_or(&Value::Null)).trim().to_string(),
        ))
    });
    ...
    t.add_func("trimSuffix", |a| {
        ...
        Ok(Value::String(
            v.strip_suffix(&suf).unwrap_or(&v).to_string(),
        ))
    });
    t.add_func("trimPrefix", |a| {
        ...
        let v = s(a.get(1).unwrap_or(&Value::Null));
        Ok(Value::String(
            v.strip_prefix(&pre).unwrap_or(&v).to_string(),
        ))
    });
    ...
    t.add_func("trunc", |a| {
        ...
        let out: String = if n >= 0 {
            ...
            let k = (-n) as usize;
            v.chars()
                .skip(v.chars().count().saturating_sub(k))
                .collect()
        };
        Ok(Value::String(out))
    });
    t.add_func("contains", |a| {
        Ok(Value::Bool(
            s(a.get(1).unwrap_or(&Value::Null)).contains(&s(a.first().unwrap_or(&Value::Null))),
        ))
    });
    t.add_func("hasPrefix", |a| {
        Ok(Value::Bool(
            s(a.get(1).unwrap_or(&Value::Null)).starts_with(&s(a.first().unwrap_or(&Value::Null))),
        ))
    });
    t.add_func("hasSuffix", |a| {
        Ok(Value::Bool(
            s(a.get(1).unwrap_or(&Value::Null)).ends_with(&s(a.first().unwrap_or(&Value::Null))),
        ))
    });
    t.add_func("indent", |a| {
        let n = a.first().and_then(|v| v.as_i64()).unwrap_or(0).max(0) as usize;
        Ok(Value::String(indent(
            &s(a.get(1).unwrap_or(&Value::Null)),
            n,
            false,
        )))
    });
    ...
    t.add_func("toYaml", |a| {
        Ok(Value::String(to_yaml(a.first().unwrap_or(&Value::Null))))
    });
    t.add_func("toJson", |a| {
        Ok(Value::String(
            serde_json::to_string(a.first().unwrap_or(&Value::Null)).unwrap_or_default(),
        ))
    });
    t.add_func("b64enc", |a| {
        Ok(Value::String(b64(
            s(a.first().unwrap_or(&Value::Null)).as_bytes()
        )))
    });
    t.add_func("b64dec", |a| {
        Ok(Value::String(
            String::from_utf8_lossy(&b64d(&s(a.first().unwrap_or(&Value::Null)))).into_owned(),
        ))
    });
    ...
    t.add_func("empty", |a| {
        Ok(Value::Bool(!truthy(a.first().unwrap_or(&Value::Null))))
    });
    t.add_func("coalesce", |a| {
        Ok(a.iter().find(|v| truthy(v)).cloned().unwrap_or(Value::Null))
    });
    ...
    t.add_func("add", |a| {
        Ok(Value::from(
            a.iter().filter_map(|v| v.as_i64()).sum::<i64>(),
        ))
    });
    t.add_func("sub", |a| {
    ...
    t.add_func("mul", |a| {
        Ok(Value::from(
            a.iter().filter_map(|v| v.as_i64()).product::<i64>(),
        ))
    });
    t.add_func("div", |a| {
        let x = a.first().and_then(|v| v.as_i64()).unwrap_or(0);
    ...
    t.add_func("int", |a| {
        Ok(Value::from(a.first().and_then(coerce_i64).unwrap_or(0)))
    });
    // collections
    ...
    t.add_func("kindOf", |a| {
        Ok(Value::String(
            kind_of(a.first().unwrap_or(&Value::Null)).into(),
        ))
    });
}
