...
src/routes.rs      hyper 1.x handlers: GET /api/settings, PATCH /api/settings
                   — the COMPONENT STANDARD for Rust HTTP is hyper (axum
                   hosts can mount these too; axum is hyper underneath)
...
1. Copy `src/*.rs` into your Rust crate (e.g. `server/src/`); add deps:
