...
export interface Transport {
  ...
  patch: (body: Record<string, string>) => Promise<unknown>;
}

function fetchTransport(apiBase: string): Transport {
...
function ConnectionRow({ conn, transport, onSaved }: { conn: Connection; transport: Transport; onSaved: () => void }) {
  ...
  const save = async () => {
    ...
    setError(null);
    try {
      await transport.patch({ [conn.key]: value.trim() });
      setValue('');
      onSaved();
    } catch (e) {
...
function InputRow({
  label, settingKey, initialValue, placeholder, note, transport, onSaved,
}: {
  label: string; settingKey: string; initialValue: string; placeholder: string;
  note?: string; transport: Transport; onSaved: () => void;
}) {
    ...
    <div>
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={async () => {
            setSaving(true);
            try { await transport.patch({ [settingKey]: value.trim() }); onSaved(); } finally { setSaving(false); }
          }}
          disabled={saving}
          className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 rounded text-sm font-medium text-white disabled:opacity-50"
        >
...
export default function ModelConfig({
  ...
  const t = transport ?? fetchTransport(apiBase);
  const [settings, setSettings] = useState<SettingsPayload | null>(null);

  const load = useCallback(async () => {
    setSettings(await t.get());
    ...
    <div className="space-y-8">
      <section className="bg-gray-800 border border-gray-700 rounded-lg p-5">
        ...
        {settings.connections.map((conn) => (
          <ConnectionRow key={conn.key} conn={conn} transport={t} onSaved={load} />
        ))}
      </section>
      ...
      <section className="bg-gray-800 border border-gray-700 rounded-lg p-5 space-y-4">
        ...
        <div>
          ...
          <select
            value={provider}
            onChange={async (e) => {
              await t.patch({ model_provider: e.target.value });
              load();
            }}
