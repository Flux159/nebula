...
`install-image` decompresses straight to its destination and writes sparsely:
the shipped rootfs is 1 GiB of which ~23 MB is non-zero, so it puts ~25 MB on
disk rather than the ~3.2 GB earlier versions wrote (a staging copy plus two
dense copies). That matters most on Windows, where every written byte goes
through Defender's real-time scanner — an upgrade that took over five minutes
now takes seconds. **Upgrades run the same command**: guest images change
between releases, so ship the new `.gz` pair and re-run it. Installing over an
existing engine is safe and leaves nothing of the old image behind; stop the
engine first (`nebula down`), which `install-image` enforces.
...
### When two instances want the same port

Isolation is per-`NEBULA_HOME`, but the host's port space is shared: two
instances configured with the same `api_port`/`dns_port`/`k8s_port` are
fighting over the same three listeners. nebulad checks them **before it boots
the VM** and, by default, refuses to start — naming the port and the
`NEBULA_HOME` of the instance holding it:

```
$ nebula up
Error: nebulad failed to start:

tcp 7461 (api_port) is already in use by nebulad pid 41234
  (NEBULA_HOME=/Users/x/Library/Application Support/YourApp/nebula)
...
```

A clear failure at `up` is cheaper than the alternative it replaces: an
instance that bound what it could, warned about the rest, reported itself
healthy, and broke a `docker build` ten minutes later with an unrelated-looking
gRPC error.

If your embedder would rather come up than match a number, set
`port_conflict = "auto"` (or `NEBULA_PORT_CONFLICT=auto`) and nebulad binds the
next free port instead, logging which. Read the effective values back from
`/v1alpha1/status` — never assume the configured ones.

Sharing a `dns_zone` across instances is legal but ambiguous: container names
resolve against whichever resolver answers first. nebulad warns when it finds
a peer instance using the same zone.
